...
class TestRequestGeneratorInit(unittest.TestCase):
    def test_valid_initialization(self):
        ...
        self.assertEqual(rg.run_id, "run_1")

    def test_invalid_rate_type(self):
        with self.assertRaises(TypeError):
            RequestGenerator(rate="fast", width=10, height=10, start_id=0, run_id="x")

    def test_negative_rate(self):
        with self.assertRaises(ValueError):
            RequestGenerator(rate=-1, width=10, height=10, start_id=0, run_id="x")

    def test_invalid_width_height_type(self):
        with self.assertRaises(TypeError):
            RequestGenerator(rate=1, width="wide", height=10, start_id=0, run_id="x")

    def test_negative_width_height(self):
        with self.assertRaises(ValueError):
            RequestGenerator(rate=1, width=-1, height=10, start_id=0, run_id="x")

    def test_invalid_start_id_type(self):
        with self.assertRaises(TypeError):
            RequestGenerator(rate=1, width=10, height=10, start_id="0", run_id="x")
...
class TestRequestGeneratorMaybeGenerate(unittest.TestCase):
    ...
    def test_generate_zero_requests_when_rate_zero(
        ...
        rg = RequestGenerator(rate=0, width=10, height=10, start_id=0, run_id="run")
        result = rg.maybe_generate(time=0)

        self.assertEqual(result, [])
        mock_event_manager.assert_called_once_with("run")

    def test_negative_time_raises(self):
        rg = RequestGenerator(rate=1, width=10, height=10, start_id=0, run_id="run")
        with self.assertRaises(ValueError):
    ...
    def test_generate_exact_base_requests(
        ...
        rg = RequestGenerator(rate=2.0, width=10, height=10, start_id=10, run_id="run")
    ...
    def test_fractional_rate_probability_adds_extra_request(
        ...
        rg = RequestGenerator(rate=1.5, width=10, height=10, start_id=0, run_id="run")
    ...
    def test_request_fields_are_correct(
        ...
        rg = RequestGenerator(rate=1, width=10, height=10, start_id=42, run_id="run")
        ...
        self.assertEqual(req.run_id, "run")
